...
class Visformer_DCMA_APCM(nn.Module):
    """
    Visformer_DCMA_APCM类：用于以APCM（Adaptive Prototype Construction Module）为核心的Few-Shot图像分类推理
    ...
    def _forward_from_feats(self, data_shot: torch.Tensor, data_query: torch.Tensor) -> torch.Tensor:
        ...
        for j in range(way):
            ...
            # strict: feed RAW flattened distance matrix (NO softmax(-d))
        ...
        # =====================================================================
        # Stage B: Z_j, phi(Z_j), select H_j, build RAW dis, MLP3, Eq.(18) union + mean
        # =====================================================================
        ...
        for j in range(way):
            # ---- build Z_j with strict semantics ----
            Z_parts = []
