...
class Curvature_generation_Visformer_Euclidean(nn.Module):
    """
    Euclidean-distance (L2) version of your current Curvature_generation_Visformer.
    Only replaces all hyperbolic-distance-related parts with Euclidean distances,
    and replaces hyperbolic mean with Euclidean weighted mean (to keep the space closed).
    """
    def __init__(self, args):
        super().__init__()
        self.args = args
    ...
    def forward(self, shot, query):
        ...
        rerank_data = rerank_data.repeat(self.args.multihead, 1)  # [multihead*?, dim]
        ...
        dis_mat = torch.zeros(self.args.way, rerank_num, device=device)  # [way, rerank_num]
        proto_p = torch.zeros(mean_proto_category.shape[0], self.args.dim, device=device)  # [way, dim]

        # Keep your projection special-cases EXACTLY
        ...
        if self.args.backbone == "convnet" and self.args.setting == "transductive" and self.args.shot == 1:
            ...
            data_shot_prooject_q = self.proj_q(rerank_data)          # [rerank_num, dim]
        ...
        for i in range(self.args.way):
            if self.args.shot == 1:
                ...
                # (1) Replace hyperbolic support distance matrix with Euclidean distance matrix
                support_dis_mat = self._cdist(data_shot_i, data_shot_i)  # [shot, shot]
                ...
                weight_data_support_i = data_shot_i * support_weight_i.squeeze(0).unsqueeze(1)  # [shot, dim]

                # (2) Replace hyperbolic mean with Euclidean weighted mean
                proto_i = weight_data_support_i.mean(dim=0, keepdim=True)  # [1, dim]
                proto_p[i] = proto_i.squeeze(0)

            # (3) Replace proto-to-rerank distances with Euclidean distances
            support_i = data_shot_prooject_q  # [rerank_num, dim]
            dis_mat[i] = self._cdist(proto_i, support_i).squeeze(0)  # [rerank_num]

        # ------------------------ Rerank / context-aware rectification ------------------------
        _, indices = torch.sort(dis_mat)  # [way, rerank_num]

        test_proto = torch.zeros_like(proto_p)  # [way, dim]

        for i in range(self.args.way):
            n_i_d = dis_mat[i, indices[i, 0:self.args.rerank]]                         # [rerank]
            o_i_d = dis_mat[i, indices[i, self.args.rerank:dis_mat.shape[1]]].mean()   # scalar
            ...
            # old-new mixing term (unchanged)
            mean_i = (mean_proto_category[i] * (1 - old_new_weight) * (i_weight.shape[1] + 1)).unsqueeze(0)  # [1, dim]

            # (4) Replace hyperbolic centroid over {neighbors + old proto} with Euclidean mean
        ...
        new_dis_mat = torch.zeros(self.args.way, data_query_norm.shape[0], device=device)

        for i in range(self.args.way):
            proto_i = test_proto[i].unsqueeze(0)  # [1, dim]
            # (5) Replace hyperbolic proto-query distance with Euclidean distance
            new_dis_mat[i] = self._cdist(proto_i, data_query_norm).squeeze(0)  # [way*query]

        logits = -new_dis_mat.t()  # [way*query, way]

        if self.training:
            return logits
        else:
            support_feats = data_shot.reshape(-1, data_shot.size(-1))  # [way*shot, dim]
            query_feats = data_query.reshape(-1, data_query.size(-1))  # [way*query, dim]
    ...
    def fusion(self, shot, query, semantic_prompt):
        ...
        for i in range(self.args.way):
            if self.args.shot == 1:
                ...
                support_dis_mat = self._cdist(data_shot_i, data_shot_i)
                support_dis_mat = support_dis_mat.view(1, -1) / np.power(self.args.dim, 0.5)
            ...
            dis_mat[i] = self._cdist(proto_i, data_shot_prooject_q).squeeze(0)
        ...
        for i in range(self.args.way):
            new_dis_mat[i] = self._cdist(test_proto[i].unsqueeze(0), data_query).squeeze(0)
