...
parser.add_argument('--name', type=str, default='acg-dab-')
...
from model.visformer_dcma import visformer_tiny
...
def main(config):
    ...
    if args.backbone == 'visformer':
        ...
        #                                                 torch.nn.Sigmoid(),)
        model.dab_block = DAB(args)
        model.gate = torch.nn.Parameter(torch.tensor(0.))
    ...
    # 1) 中等学习率组：args.lr
    mid_lr_params_id = []
    mid_lr_params_id += [id(param) for param in model.t2i.parameters()]
    mid_lr_params_id += [id(param) for param in model.t2i2.parameters()]
    mid_lr_params_id += [id(model.gate)]
    mid_lr_params_id += [id(model.log_tau)]
    ...
    for param in model.parameters():
        pid = id(param)
        if pid not in mid_lr_params_id:
