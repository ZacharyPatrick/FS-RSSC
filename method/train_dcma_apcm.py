...
parser.add_argument('--name', type=str, default='acg-can-dab-')
...
from model.visformer_dcma_apcm import Visformer_DCMA_APCM
...
def main(config):
    ...
    if args.backbone == 'visformer':
        ...
        model = Visformer_DCMA_APCM(args=args)
    else:
        ...
        if epoch % ef_epoch == 0 or epoch== 1:
            ...
            with torch.no_grad():
                ...
                for episode in eval_pbar:
                    ...
                    # logits = F.normalize(query, dim=-1) @ F.normalize(gen_support, dim=-1).t()
                    # ori_logits, _, _ = model(sup1, que)
                    gen_logits = model.fusion(sup, que, text_features)
